...
from scraper import scrape_techcombank_monthly_reports
...
from client_dashboard_ui import (
    ...
    render_html_block,
    render_sidebar_html,
    render_table_header_divider,
    render_table_row_divider,
)
from utils import (
...
def _render_client_picker(clients: list[Client]) -> None:
    """Let user pick one client to open the dedicated detail view."""
    if not clients:
        return
    st.subheader("Open client")
    labels = [
        f"{c.name} · {iso_date_or_empty(c.birthday) or 'No birthday'}"
        for c in clients
    ]
    pick_idx = st.selectbox(
        ...
        key="clients_view_pick_idx",
    )
    if st.button("Open client dashboard", type="primary", key="open_selected_client"):
        st.session_state.selected_client_id = int(clients[pick_idx].id)
        st.rerun()
...
"""
    ...
    ensure_viewport_detected()
    mobile_ui = is_mobile_ui()

    st.sidebar.header("Navigation")
    tab = st.sidebar.radio("Go to", ["Clients", "Reminders", "Market News", "Settings"], index=0)
    st.sidebar.caption(f"Layout: **{device_layout_label(mobile=mobile_ui)}** (auto)")
    ...
    if tab == "Clients":
        ...
        with get_session(DB_URL) as session:
            ...
            if st.session_state.selected_client_id is None:
                _render_client_picker(clients[:clients_expander_limit])
                return

            c = session.get(Client, int(st.session_state.selected_client_id))
            if c is None:
                st.session_state.selected_client_id = None
                st.rerun()

            if st.button("← Back to all clients", key="back_to_clients_list"):
                st.session_state.selected_client_id = None
                st.session_state.client_edit_id = None
                st.session_state.client_delete_confirm_id = None
                st.rerun()
            ...
            st.subheader(c.name)
            if c.birthday:
                st.caption(f"Birthday: {iso_date_or_empty(c.birthday)}")
            ...
            if client_tab == "Overview":
                side_html = render_sidebar_html(
                    ...
                    realized_pnl=realized_past_sum,
                ...
                st.caption(f"Display: **VND** · FX 1 USD = {fx_rate:,.0f} VND")

            elif client_tab == "Portfolio":
                if df.empty:
                    st.info("No active investments for this client.")
                else:
                    st.caption("Active investments grouped into Equities and Debts.")
                    ...
                    if catalog_drill:
                        ...
                        drill_header = st.columns([1.8, 0.45] + [1.35] * len(detail_visible_cols) + [0.7, 0.45])
                        drill_header[0].markdown("**Asset Type**")
                        drill_header[1].markdown("** **")
                        for hi, name in enumerate(detail_visible_cols):
                            drill_header[hi + 2].markdown(f"**{name}**")
                        drill_header[-2].markdown("**Done**")
                        drill_header[-1].markdown("** **")
                        render_table_header_divider(st)
                        for inv, row in detail_entries:
                            ...
                            if row_cols[1].button(
                                "✏",
                                key=f"drill_edit_{c.id}_{inv.id}",
                            ...
                            for j, name in enumerate(detail_visible_cols):
                                ...
                                row_cols[j + 2].write(_fmt_cell(name, value_for_cell))
                            done_key = f"done_drill_{c.id}_{inv.id}"
                            ...
                            if done_checked != bool(getattr(inv, "is_done", False)):
                                if done_checked:
                                    ...
                                    if inv_to_update:
                                        ...
                                        session.commit()
                                        st.rerun()
                            if row_cols[-1].button("✖", key=f"drill_del_{c.id}_{inv.id}", type="primary"):
                                st.session_state.pending_delete_investment_id = inv.id
                                st.rerun()
                            render_table_row_divider(st)
...
""",
        ...
        with get_session(DB_URL) as session:
            render_intel_terminal(
                session,
                mobile_ui=mobile_ui,
                techcom_reports_fn=cached_techcom_reports,
            )
